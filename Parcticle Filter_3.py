...
def drawCross(img, particles, r, g, b):
    d = 5
    t = 2
    LINE_AA = cv2.LINE_AA
    color = (r, g, b)
    ctrx = int(sum(particles[:, 0])/len(particles))
    ctry = int(sum(particles[:, 1])/len(particles))
...
def update(particles, weights, z, R, landmarks):
    ...
    for i, landmark in enumerate(landmarks):
        ...
        weights *= scipy.stats.norm.pdf(z[i],distance, R)#正态分布的概率密度函数,distance期望，R标准差，pdf(z[i])为概率密度函数
...
while (1):
    ...
    drawCross(img, particles, r=255, g=0, b=0)
